...
export class UpdateBlogCommand implements ApplicationService<UpdateBlogDto, UpdateBlogResponse> {
  ...
  async execute(data: UpdateBlogDto): Promise<Result<UpdateBlogResponse>> {
    ...
    if (!blog) 
      return Result.failure<UpdateBlogResponse>( new BlogNotFound());
    ...
    await this.repository.createBlog(blog);
