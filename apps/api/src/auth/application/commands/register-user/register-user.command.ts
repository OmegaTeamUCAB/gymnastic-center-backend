...
export class RegisterUserCommand implements ApplicationService<RegisterDto, RegisterResponse> {
    constructor(
    ...
    public async execute(data: RegisterDto): Promise<Result<RegisterResponse>> {
        ...
        const newUser = new AuthUser(UUIDService.generateUUID(), data.email, hashedPassword);
        await this.authRepository.save(newUser);
        const payload = { id: newUser.id };
        return Result.success<RegisterResponse>({ id: this.jwtService.generateToken(payload) });
    }
}